...
class RegionAdapter(
    ...
    private val onSelected: ((String, Int) -> Unit)? = null
) :
    ...
    inner class ViewHolder(val binding: ItemRcvTvBinding) :
        ...
        private fun setClick(tv: TextView) {
            tv.setOnClickListener {
                ...
                onSelected?.invoke(data[pos], pos)
            }
        }
    }
    ...
    fun setItems(newItems: List<String>, keepSelection: Boolean = true, triggerCallback: Boolean = true) {
        val oldSel = selectedPosition
        data = newItems
        selectedPosition = if (keepSelection) clamp(oldSel, data.size) else 0
        notifyDataSetChanged()
        if (triggerCallback && data.isNotEmpty() && selectedPosition != RecyclerView.NO_POSITION) {
            onSelected?.invoke(data[selectedPosition], selectedPosition)
        }
    }
    ...
    fun setSelectedPosition(newPos: Int, triggerCallback: Boolean = false) {
        ...
        if (triggerCallback) onSelected?.invoke(data[newPos], newPos)
    }
}